...
public abstract class YouTubeExtractor extends AsyncTask<String, Void, YouTubeExtractor.YoutubeMediaUrlInfo> {
    ...
    @Override
    protected void onPostExecute(YoutubeMediaUrlInfo ytMediaUrlInfo) {
        onExtractionComplete(ytMediaUrlInfo, videoMeta);
    }
    ...
    protected abstract void onExtractionComplete(YoutubeMediaUrlInfo ytMediaUrlInfo, VideoMeta videoMeta);

    @Override
    protected YoutubeMediaUrlInfo doInBackground(String... params) {
        videoID = null;
    ...
    public class YoutubeMediaUrlInfo {

        @Nullable
        private String dashMpdUrl;

        @Nullable
        private SparseArray<YtFile> streamUrls;

        public YoutubeMediaUrlInfo(@Nullable String dashMpdUrl, @Nullable SparseArray<YtFile> streamUrls) {
        ...
        @Nullable
        public SparseArray<YtFile> getStreamUrls() {
            return streamUrls;
        }
    }

    private YoutubeMediaUrlInfo getStreamUrls() throws IOException, InterruptedException {
        ...
        if(videoMeta.isLiveStream()){
            ...
            if(mat.find()) {
                ...
                return new YoutubeMediaUrlInfo(dashMpdUrl, ytFiles);
            }
            return null;
        }

        Log.d(LOG_TAG, "304 dashMpdUrl=" + dashMpdUrl);
            ...
            if (parseDashManifest) {
                ...
                if (mat.find()) {
                    ...
                    if (mat.find()) {
                        ...
                        dashMpdUrl = null;
                        Log.d(LOG_TAG, "372 dashMpdUrl=" + dashMpdUrl);
                    }
                }
                Log.d(LOG_TAG, "374 dashMpdUrl=" + dashMpdUrl);
            }
        } else {
            if (parseDashManifest) {
                ...
                if (mat.find()) {
                    ...
                    Log.d(LOG_TAG, "382 dashMpdUrl=" + dashMpdUrl);
                }
            }
            Log.d(LOG_TAG, "385 dashMpdUrl=" + dashMpdUrl);
            ...
            if (signature == null) {
                ...
                for (int i = 0; i < encSignatures.size() && i < sigs.length; i++) {
                    ...
                    if (key == 0) {
                        ...
                        Log.d(LOG_TAG, "457 dashMpdUrl=" + dashMpdUrl);
                    } else {
        ...
        Log.d(LOG_TAG, "468 dashMpdUrl=" + dashMpdUrl);
